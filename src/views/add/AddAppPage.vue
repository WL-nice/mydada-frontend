...
import { AppAddRequest, AppControllerService, AppVO } from "../../../generated";
...
const loadData = async () => {
  ...
  const res = await AppControllerService.getAppVo(props.id as any);
...
const handleSubmit = async () => {
  ...
  // 如果是修改
  if (props.id) {
    res = await AppControllerService.editApp({
      id: props.id as any,
      ...form.value,
    });
  } else {
    // 创建
    res = await AppControllerService.addApp(form.value);
  }
  if (res.code === 0) {
