...
import { App, AppControllerService, AppQueryRequest } from "../../../generated";

const formSearchParams = ref<AppQueryRequest>({});
...
const loadData = async () => {
  const res = await AppControllerService.listAppByPage(searchParams.value);
  if (res.code === 0) {
 ...
 * @param record
 */
const doDelete = async (record: App) => {
  if (!record.id) {
    return;
  }

  const res = await AppControllerService.deleteApp(record.id);
  if (res.code === 0) {
    loadData();
  } else {
...
const doReview = async (
  ...
  const res = await AppControllerService.doAppReview({
    id: record.id,
    reviewStatus,
    reviewMessage,
  });
