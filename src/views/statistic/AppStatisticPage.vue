...
import "echarts";
import {
  AppAnswerResultCountDTO,
  AppStatisticControllerService,
  UserAnswerCountDTO,
} from "../../../generated";

const appAnswerCountList = ref<UserAnswerCountDTO[]>([]);
...
const loadAppAnswerCountData = async () => {
  const res = await AppStatisticControllerService.getUserAnswerCount();
  if (res.code === 0) {
...
const loadAppAnswerResultCountData = async (appId: string) => {
  ...
  const res = await AppStatisticControllerService.getAppAnswerResultCount(
    appId as any
  );
  if (res.code === 0) {
